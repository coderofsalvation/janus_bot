...
class BotProtocol(LineReceiver):
    ...
    def _command_line_input(self):
        ...
        Will take user input using input and pass it as a chat or command
        """
        while True:
            chat = input(":")
    ...
    def tick(self):
        ...
        if self.state == STATES.STAYING:
            self.sendLine(json.dumps({"method": "move", "data": self.getAvatarString()}).encode('utf-8'))
    ...
    def do_follow(self, new_pos):
        ...
        if dist > float(self.avatar_scale) * FOLLOW_DIST:
            ...
            self.sendLine(json.dumps({"method": "move", "data": to_send}).encode('utf-8'))
        else:
            self.sendLine(json.dumps({"method": "move", "data": self.getAvatarString()}).encode('utf-8'))
            ...
            if 'data' in msg and 'userId' in msg['data'] and msg['data']['userId'] == self.following:
                if msg['method'] == 'user_moved' :
                    ...
                    if msg['data']['roomId'] != self.room_id:
                        self.sendLine(json.dumps({'method': 'subscribe', 'data': {'roomId': msg['data']['roomId']}}).encode('utf-8'))
                        self.sendLine(json.dumps({'method': 'enter_room', 'data': {'roomId': msg['data']['roomId']}}).encode('utf-8'))
                    ...
                    if 'newRoomId' in msg['data'] is None:
                        ...
                        self.sendLine(json.dumps({'method': 'subscribe', 'data': {'roomId': msg['data']['newRoomId']}}).encode('utf-8'))
                        self.sendLine(json.dumps({'method': 'enter_room', 'data': {'roomId': msg['data']['newRoomId']}}).encode('utf-8'))
    ...
    def sendChat(self, text, listen_to_self=False):
        ...
        """
        self.sendLine(json.dumps({"method": "chat", "data": str(text)}).encode('utf-8'))
        if listen_to_self:
...
if __name__ == "__main__":
    ...
    reactor.connectTCP("presence.janusvr.com", 5566, BotFactory())
    reactor.run()
